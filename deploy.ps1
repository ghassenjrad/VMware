    ...
    foreach ($dc in Get-Datacenter -Name ($basenameDC + "*")) {
        ...
        foreach ($esx in $vmh) {
            ...
            if ($ds.Count -eq 0) {
                ...
                New-Datastore -VMHost $esx -Name $dsName -Path $disks[1].ScsiLun -Vmfs -Confirm:$false | Out-Null
            }
        }
    }
}
        ...
        foreach ($vmh in (Get-VMHost -Location $d)) {
            ...
            if ($na.Count -gt 0) {
                ...
                if ($partitions.Count -eq 0) {
                    Write-Host "Creating vSAN Disk Group..." -ForegroundColor $DefaultColor
                    try {
                        # Start the task asynchronously
                        $task = New-VsanDiskGroup -VMHost $vmh -DataDiskCanonicalName $dataDisk -SsdCanonicalName $cacheDisk -RunAsync

                        # Use Wait-Task to properly wait for its completion.
                        # This cmdlet handles the lifecycle of the task object better than a manual loop.
                        # Increase TimeoutSeconds if your operations typically take longer.
                        $task | Wait-Task 40 -ErrorAction Stop # Wait up to 40 seconds

                        Write-Host "vSAN Disk Group created successfully for $($vmh.Name)." -ForegroundColor Green
                    }
                    catch {
                        Write-Host "Continue" -ForegroundColor Gray
                        # If you want to continue processing other hosts even if one fails:
                        continue
                        # If failure on one host should stop the entire script:
                        # throw $_
                    }
                }
            }
